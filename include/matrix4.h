...
namespace Earth
{
	namespace core
	{
		...
		*/
			...
			f32 fieldOfViewRadians, f32 aspectRatio, f32 zNear, f32 zFar)
		{
			...
			_EARTH_DEBUG_BREAK_IF(aspectRatio == 0.f); //divide by zero
			const T w = static_cast<T>(h / aspectRatio);

			_EARTH_DEBUG_BREAK_IF(zNear == zFar); //divide by zero
			M[0] = w;
			M[1] = 0;
			M[2] = 0;
			M[3] = 0;
	...
	//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
		...
		{
			_EARTH_DEBUG_BREAK_IF(widthOfViewVolume == 0.f); //divide by zero
			_EARTH_DEBUG_BREAK_IF(heightOfViewVolume == 0.f); //divide by zero
			_EARTH_DEBUG_BREAK_IF(zNear == zFar); //divide by zero
